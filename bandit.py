...
def make_adversarial_examples(image, true_label, args, model_to_fool, IMAGENET_SL):
    ...
    while not ch.any(total_queries > args.max_queries):
        ...
        if not args.nes:
            ...
            l1 = criterion(model_to_fool(image + args.fd_eta*q1/norm(q1)), true_label) # L(prior + c*noise)
            l2 = criterion(model_to_fool(image + args.fd_eta*q2/norm(q2)), true_label) # L(prior - c*noise)
            ...
            for _ in range(args.gradient_iters):
                ...
                est_deriv = (criterion(model_to_fool(image + args.fd_eta*exp_noise), true_label) - criterion(model_to_fool(image - args.fd_eta*exp_noise), true_label)) / args.fd_eta
        ...
        ## Logging stuff
        new_losses = criterion(model_to_fool(image), true_label)
...
def main(args, model_to_fool, dataset_size):
    dataset = datasets.CIFAR10(root = "~/Documents/data", train = False, transform = transforms.Compose([transforms.ToTensor()]))
